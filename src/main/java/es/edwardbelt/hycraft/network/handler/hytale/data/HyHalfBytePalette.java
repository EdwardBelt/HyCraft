...
public class HyHalfBytePalette implements HyPalette {
    private final Map<Byte, Integer> internalToExternal = new HashMap<>();
    private final byte[] blocks = new byte[16384];
    ...
    public int getBlock(int index) {
        byte paletteIndex = getNibble(blocks, index);
    ...
    private static byte getNibble(byte[] array, int index) {
        ...
        if (nibbleIndex == 0) {
            return (byte) ((array[byteIndex] >> 4) & 0x0F);
        } else {
            return (byte) (array[byteIndex] & 0x0F);
        }
    }
}